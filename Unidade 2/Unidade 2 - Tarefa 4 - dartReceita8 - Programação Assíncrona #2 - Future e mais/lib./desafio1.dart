...
class _DataTableWidgetState extends State<DataTableWidget> {
  ...
  void onColumnSort(int columnIndex) {
    if (columnIndex >= 0 && columnIndex < widget.columnNames.length) {
      String columnName = widget.columnNames[columnIndex];
      List<dynamic> sortedList = widget.jsonObjects.toList();

      sortedList.sort((a, b) {
        dynamic aValue = a[columnName];
        dynamic bValue = b[columnName];
      ...
      setState(() {
        widget.jsonObjects.clear();
        widget.jsonObjects.addAll(sortedList);
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: DataTable(
        columns: getColumns(),
        rows: widget.jsonObjects.map((jsonObject) {
          List<DataCell> cells = widget.columnNames.map((columnName) {
            dynamic value = jsonObject[columnName];
